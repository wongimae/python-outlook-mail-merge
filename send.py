...
for index, row in df.iterrows(): #Loops through each row in the dataframe
    ...
    if (pd.isnull(email) or pd.isnull(subject) or pd.isnull(content) or pd.isnull(voucher)): #Skips over rows where one of the cells in the three main columns is blank
        continue
    ...
    newMail.HTMLbody = (r"" + content +  "" + voucher) #Sets the mail's body to 'body' variable
